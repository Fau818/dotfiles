...
# \     If the app is in white_list or is Finder, it will focus to the last managed window when the window losed focus.
...
white_list=$1 && ([[ "$app_name" =~ "$white_list" ]] || [ "$app_name" = "Finder" ]) && yabai -m window --focus last

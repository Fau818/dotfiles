...
function update_space_type() {
  ...
  if [[ "$SELECTED" == 'true' ]]; then
    ...
    if [[ "$space_type" == 'float' ]];   then color="$FLOAT_LAYOUT_COLOR"
    elif [[ "$space_type" == 'bsp' ]];   then color="$BSP_LAYOUT_COLOR"
    elif [[ "$space_type" == 'stack' ]]; then color="$STACK_LAYOUT_COLOR"
    fi
  else border_color="$UNFOCUS"
  fi
...
function mouse_clicked() {
  if [[ "$MODIFIER" == 'shift' ]]; then
    ...
    elif [[ "$BUTTON" == 'left' ]]; then  yabai -m space --focus "$SID"
    elif [[ "$BUTTON" == 'right' ]]; then yabai -m space --destroy "$SID"
  fi
}
